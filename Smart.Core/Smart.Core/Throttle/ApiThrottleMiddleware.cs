   ...
   public class ApiThrottleMiddleware
    {
        ...
        private async Task<(bool result, ThrottleValve valve)> CheckAsync(HttpContext context)
        {
            //全局阀门
            foreach (var valve in _options.Global.Valves.OrderByDescending(x => x.Priority))
            {
                ...
                {
                    //黑名单
                    var wl = await _cache.GetRosterListAsync(RosterType.BlackList, CommonHelper.GlobalApiKey, valve.Policy, valve.PolicyKey);
                ...
                {
                    //白名单
                    var wl = await _cache.GetRosterListAsync(RosterType.WhiteList, CommonHelper.GlobalApiKey, valve.Policy, valve.PolicyKey);
                    if (wl.Any(x => string.Equals(x.Value, policyValue)))
                    {
                        return (true, null);
                    }
                ...
                else if (valve is RateValve rateValve)
                {
                    ...
                    //判断是否过载
                    long count = await _cache.GetApiRecordCountAsync(CommonHelper.GlobalApiKey, rateValve.Policy, rateValve.PolicyKey, policyValue, DateTime.Now, rateValve.Duration);
        ...
        private async Task SaveAsync(HttpContext context)
        {
            ...
            foreach (RateValve valve in _options.Global.Valves.Where(x => x is RateValve))
            {
                ...
                //保存记录
                await _cache.AddApiRecordAsync(CommonHelper.GlobalApiKey, valve.Policy, valve.PolicyKey, policyValue, nowTime, valve.Duration);
            }
        }
    }
}
