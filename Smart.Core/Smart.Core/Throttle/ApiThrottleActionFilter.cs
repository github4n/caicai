...
namespace Smart.Core.Throttle
{
    public class ApiThrottleActionFilter : IAsyncActionFilter, IAsyncPageFilter
    {
        ...
        private IEnumerable<ThrottleValve> _valves;

        /// <summary>
        /// 处理接口
        /// </summary>
        /// <returns></returns>
        private async Task<(bool result, ThrottleValve valve)> HandleAsync(FilterContext context)
        {
            //预处理数据
            ...
            _valves = method.GetCustomAttributes<ThrottleValve>(true);

            //检查是否过载
            var result = await CheckAsync(context);
            if (result.result)
            {
                context.HttpContext.Request.Headers[CommonHelper.HeaderStatusKey] = "1";
                //保存记录
                await SaveAsync(context);
            }
            else
            {
                context.HttpContext.Request.Headers[CommonHelper.HeaderStatusKey] = "0";
            }
        ...
        /// </summary>
        /// <returns></returns>
        private async Task<(bool result, ThrottleValve valve)> CheckAsync(FilterContext context)
        {
            //循环验证是否过载
