...
{
   public class FC3D_opencai
    {
        public static List<string> Analysis()
        {
            List<string> list = new List<string>();
            var client = NetHelper.GetHttpClient("FC3D_opencai");
            {
                var result = client.GetAsync("http://t.apiplus.net/newly.do?code=fc3d&format=json").Result;
