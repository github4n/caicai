...
{
    public class PL3_opencai
    {
        public static List<string> Analysis()
        {
            List<string> list = new List<string>();
            var client = NetHelper.GetHttpClient("PL3_opencai");
            {
                var result = client.GetAsync("http://t.apiplus.net/newly.do?code=pl3&format=json").Result;
