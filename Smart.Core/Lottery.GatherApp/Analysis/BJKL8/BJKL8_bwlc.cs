...
{
    public class BJKL8_bwlc
    {
        public static List<string> Analysis()
        {
            List<string> list = new List<string>();
            var client = NetHelper.GetHttpClient("BJKL8_bwlc");
            {
                var result = client.GetAsync("http://www.bwlc.gov.cn/bulletin/keno.html").Result;
                if (result.IsSuccessStatusCode)
                {
                    ...
                    var matchs = Regex.Matches(context, "<td>(?<isses>.*?)</td>.*?<td>(?<code>.*?)</td>.*?<td>(?<code1>.*?)</td>.*?<td>.*?</td>.*?", RegexOptions.IgnoreCase | RegexOptions.Singleline);
                    int i = 0;
                    foreach (Match mc in matchs)
                    {
                        ...
                        var code1 = mc.Groups["code1"].Value;
                        list.Add($"{isses}|{code}+{code1}");
                        i++;
                    }
                }
            }
