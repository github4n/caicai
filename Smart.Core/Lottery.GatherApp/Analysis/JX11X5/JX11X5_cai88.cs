...
namespace Lottery.GatherApp
{
    public class JX11X5_cai88
    {
        ...
        public List<string> Analysis()
        {
            ...
            var client = NetHelper.GetHttpClient("JX11X5_cai88");
            {
                var result = client.GetAsync("http://cai88.com/api/getgame.action?type=bozhong31").Result;
                if (result.IsSuccessStatusCode)
                {
                    ...
                    {
                        var preDrawIssue = item["code"].ToString();//开奖号
                        var preDrawTime = data1;//开奖时间
                        var preDrawCode = item["issue"].ToString();//开奖期号
