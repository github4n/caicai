...
{
    public class GD11X5_opencai
    {
        public static List<string> Analysis()
        {
            List<string> list = new List<string>();
            var client = NetHelper.GetHttpClient("GD11X5_opencai");
            {
                var result = client.GetAsync("http://t.apiplus.net/newly.do?code=gd11x5&format=json").Result;
